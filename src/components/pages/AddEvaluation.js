...
class AddEvaluation extends Component {
  ...
  assessmentSubmit = async (event) => {
    ...
    for (
      let index = 0;
      index < this.state.EvaluationAssessments.length;
      index++
    ) {
      total =
        parseInt(this.state.EvaluationAssessments[index].effectRation) + total;
    }
    ...
    if (this.props.token) {
      ...
      if (history.location.state) {
        ...
        await axios
          .get(
            Config.ApiUrl +
              'api/evaluation/getbyid?evaluationid=' +
        ...
        await axios
          .get(
            Config.ApiUrl +
              'api/evaluationAssessment/getListbyevaluationId?evaluationAssessmentid=' +
              history.location.state.id
          )
          .then((e) => {
  ...
  render() {
    return (
      <div>
        {this.state.IsAdd && (
          <div className="content-title">Add Evaluation</div>
        )}
        {!this.state.IsAdd && (
          <div className="content-title"> Update Evaluation</div>
        )}
        ...
        {this.state.Id !== 0 && (
          <div>
            <div className="content-title mt-5">Assessment </div>
              ...
              <tbody>
                {this.state.EvaluationAssessments.map((e) => (
                  <tr key={e.id}>
                    <td>
                      {
                        this.state.Assessments[
                          this.state.Assessments.findIndex(
                            (x) => x.id === parseInt(e.assessmentId)
                          )
                        ].name
                      }
                    </td>
                    ...
                    <td>
                      <Button
                        onClick={() => this.deleteEvaluationAssessment(e)}
                      >
                        Delete
                      </Button>
                    </td>
